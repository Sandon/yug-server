...
module.exports = async function processOneReq (config, req) {
  ...
  //hostCofig = JSON.parse(JSON.stringify(hostCofig)) // clone the config
  let rewriteRules = hostCofig.rewrite || []
  
  // If there are rules match the path, respond with the first succeed.
  // If all are failed, respond with a warning
  let matched = false, result
  const succeed = rewriteRules.find(async (rule) => {
    console.log(rule)
    ...
    if (rule.test(req.path)) {
      matched = true
      if (result = await tryRewrite(rule, req)) {
        return true
      }
    }
  })
  ...
  if (!matched) {
    // If there is no rewrite rule or the path does not match any rules,
    // yug-server serves static files from the root without any rewrites
    result = rewriteToFs(hostCofig.root)
  } else if (!succeed) {
    // If all are failed, respond with a warning
    throw new Error(`All matched rules failed! Can't get the resources specified by the rules!`)
  }
...
async function tryRewrite(rule, req) {

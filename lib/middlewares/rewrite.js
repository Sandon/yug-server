  ...
  return function (next) {
    http.get(rule.path, function (r) {
      ctx.response.set({
        ...
        'file-path': r.headers['file-path'] ? rule.path + ' -> ' + r.headers['file-path'] : rule.path
      });
    ...
    function doRewrToFs(path) {
      return function *() {
        try {
          ...
          if (stat.isFile()) {
            //console.log(rule.oldPath + ' , ' + rule.path)
            /*if ('/yugserverrewrite/css/footer.css' == ctx.url) {
              console.log('-----')
              console.log(ctx.url)
